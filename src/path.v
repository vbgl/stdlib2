...
Require Import base bool functions eqtype morenat seq.
...
Lemma pathP x p x0 :
  reflect (forall i, i < size p -> e (nth x0 (x :: p) i) (nth x0 p i))
          (path x p).
Proof.
elim: p x => [|y p IHp] x /=; first by left => i; rewrite ltn0 => /notF.
...
rewrite /= andbC; case: IHp => [fn_p | not_fn_p]; last first.
  by right=> [] [[//|n]] /seqI[<- fn_p]; case: not_fn_p; exists n.
apply: (iffP eqP) => [-> | [[] // _ /seqI[]//]].
