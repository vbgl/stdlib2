...
(**
 ...
 the br          anch of the predType match, to ensure that unifying mem A with
...
Structure registered_applicative_pred p := RegisteredApplicativePred {
  ...
  #[canonical(false)]
  _registered_applicative_pred_2 : applicative_pred_value = p
}.
Definition ApplicativePred p := RegisteredApplicativePred (eq_refl p).
...
Structure manifest_simpl_pred p := ManifestSimplPred {
  ...
  #[canonical(false)]
  _manifest_simpl_pred_2 : simpl_pred_value = SimplPred p
}.
...
Structure manifest_mem_pred p := ManifestMemPred {
  ...
  #[canonical(false)]
  _manifest_mem_pred_2 : mem_pred_value= Mem [eta p]
}.
Canonical expose_mem_pred p := ManifestMemPred (eq_refl (Mem [eta p])).
...
Structure keyed_pred (k : pred_key p) :=
  PackKeyedPred { unkey_pred :> {pred T}; #[canonical(false)] _keyed_pred_1 : unkey_pred =i p}.
...
Proof. by []. Qed.
Definition inE := (in_applicative, in_simpl, simpl_predE, simpl_pred_sortE). (* to be extended *)
