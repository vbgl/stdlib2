...
Require Import base functions bool eqtype morenat seq choice path.
...
Definition EnumMixin : forall e : seq T, axiom e -> mixin_of T :=
  let: Countable.Pack (Countable.Class _ m) as cT := T
...
Proof.
rewrite -(@eq_has _ (mem (enum A))) => [x|]; first exact: mem_enum.
...
Notation "[ 'finMixin' 'of' T 'by' <: ]" :=
    (SubFinMixin_for (Phant T) equality.eq_refl)
