...
(** Addition *)
Fixpoint addn_rec (m n: nat) : nat :=
  if m is m'.+1 then (addn_rec m' n).+1 else n.

Definition addn := nosimpl addn_rec.

Infix "+" := addn : nat_scope.
...
Proof. by elim: m => // m ih; congr (_.+1). Qed.
...
Fixpoint subn_rec (m n: nat) : nat :=
  match m, n with
  | m'.+1, n'.+1 => subn_rec m' n'
  | _, _ => m
  end.

Definition subn := nosimpl subn_rec.
Notation "m - n" := (subn m n) (at level 50, left associativity) : nat_scope.
...
Notation "m <= n" := (leq m n) (at level 70, no associativity) : nat_scope.
Notation "m < n"  := (m.+1 <= n) (at level 70, no associativity) : nat_scope.
Notation "m >= n" := (n <= m) (at level 70, no associativity, only parsing) : nat_scope.
Notation "m > n"  := (n < m) (at level 70, no associativity, only parsing)  : nat_scope.
...
Notation "m <= n <= p" := ((m <= n) && (n <= p)) (at level 70, n at next level) : nat_scope.
Notation "m < n <= p" := ((m < n) && (n <= p)) (at level 70, n at next level) : nat_scope.
Notation "m <= n < p" := ((m <= n) && (n < p)) (at level 70, n at next level) : nat_scope.
Notation "m < n < p" := ((m < n) && (n < p)) (at level 70, n at next level) : nat_scope.
...
Fixpoint muln_rec (m n: nat) {struct m} : nat :=
  if m is m'.+1 then n + muln_rec m' n else 0.
...
Notation "m ^ n" := (expn_rec m n) (at level 30, right associativity) : nat_rec_scope.
...
Fixpoint double_rec n := if n is n'.+1 then n'.*2.+2 else 0
...
Ltac nat_congr := first
 ...
 | apply: (eq_congr1 predn _)
 | apply: (eq_congr1 (addn _) _)
 | apply: (eq_congr1 (subn _) _)
 | apply: (eq_congr1 (addn^~ _) _)
 | match goal with |- (?X1 + ?X2 = ?X3) =>
     symmetry;
     rewrite -1?(addnC X1) -?(addnCA X1);
     apply: (eq_congr1 (addn X1) _);
     symmetry
   end ].
