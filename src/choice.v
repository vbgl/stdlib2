...
Require Import seq.
Import base bool functions eqtype morenat.
...
Definition opair_of_sum (s : T1 + T2) :=
  match s with Left x => (Some x, None) | Right y => (None, Some y) end.
Definition sum_of_opair p :=
  oapp (some \o @Right T1 T2) (omap (@Left _ T2) p.1) p.2.
...
Lemma treeI (x x': tree) :
  x = x' ->
  match x with
  | Leaf t => if x' is Leaf t' then t = t' else False
  | Node n s => if x' is Node n' s' then n = n' /\ s = s' else False
  end.
...
Definition tree_ind K IH_leaf IH_node :=
  ...
  | Node n f0 =>
    let fix iter_conj f : foldr (fun t => and (K t)) True f :=
        if f is t :: f' then and_intro (loop t) (iter_conj f') else prop.I
...
Fixpoint encode t : seq (nat + T) :=
  match t with
  | Leaf x => [:: Right x]
  | Node n f => Left n.+1 :: rcons (flatten (map encode f)) (Left 0)
  end.

Definition decode_step c fs := 
  match c with
  | Right x => (Leaf x :: fs.1, fs.2)
  | Left 0 => ([::], fs.1 :: fs.2)
  | Left n.+1 => (Node n fs.1 :: head [::] fs.2, behead fs.2)
  end.
...
by rewrite rcons_cat foldr_cat => -> /= /IHf/pairI[] /seqI[] /treeI[] _ -> -> ->.
Qed.
...
Record mixin_of T := Mixin {
  ...
  complete : forall P : pred T, (exists x, P x) -> exists n, find P n;
  extentional : forall P Q : pred T, P =1 Q -> find P =1 find Q
}.
...
Notation xclass := (class _ : class_of (sort cT)).
...
move=> Qm minPm Pn minQn; suffices /eqP->: m == n by move=> -> /optionI.
...
Definition choose P x0 :=
  if insub x0 : {? x | P x} is Some (exist _ x Px) then
    xchoose (@Ex _ [eta P] x Px)
  else x0.
...
- elim: {n}(dc n) Nil => [|n ns IHs] xs /=; first by case: ifP => // sPxs /optionI<-.
...
- rewrite /f /fi; case: (dc n) => [|ni [|nt []]] //=.
  ...
  by case Df: (find _ _) => //= [x] /optionI<-; have:= correct Df.
